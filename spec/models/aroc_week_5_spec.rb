...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    # ------------------------------------------------------------
    ordered_items_names = Item.joins(:orders).distinct.pluck(:name).sort
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:items).group(:name).select("users.name , count(items) as total_item_count")
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(:order_items).group("users.name, orders.id").select("users.name as user_name, orders.id as order_id, TRUNC(orders.amount / count(order_items.id)) as avg_item_cost").order("user_name desc, avg_item_cost")
    # ---------------------------------------------------------------
  ...
  it '30. returns the names of items that have been ordered without n+1 queries' do
    ...
    # ------------------------------------------------------
    orders = Order.joins(:items).pluck(:name) # Edit only this line
    # ------------------------------------------------------
